...
def collect_image_paths(data_dir):
    ...
    crawler_ = futil.FileSystemDataCrawler(data_dir, image_keys, MyFilePathGenerator(), dir_filter)
...
def load_images(image_paths):
    ...
    image_dict = {
        ...
        structure.BrainImageTypes.GroundTruth: sitk.ReadImage(image_paths[structure.BrainImageTypes.GroundTruth], sitk.sitkUInt8)
    }
...
def register_images(image_dict, atlas_img):
    ...
    registered_gt = sitk.Resample(gt_img, atlas_img, registration.transform, sitk.sitkNearestNeighbor)
...
def extract_feature_median_t1(image_dict):
    ...
    filter_ = MedianFilter()  # todo: modify here
